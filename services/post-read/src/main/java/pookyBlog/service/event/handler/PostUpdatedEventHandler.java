...
public class PostUpdatedEventHandler implements EventHandler<PostUpdatedEventPayload>{
    private final PostQueryModelRepository postQueryModelRepository;

    @Override
    public void handle(Event<PostUpdatedEventPayload> event) {
    ...
    @Override
    public boolean supports(Event<PostUpdatedEventPayload> event) {
        return EventType.POST_UPDATED == event.getType();
    }
}
